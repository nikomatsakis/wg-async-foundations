...
* Async fn in traits and especially enabling their use in `dyn Trait`
* Async closures
* Defining standard traits for portability:
    * async read
    * async write
    * async iteration
    * timers
...
* Integrating standard traits for portability into libstd
* Defining and standardizing combinators for async iterators, futures
* How to be generic over sync vs async
...
* Defining some way to create a scope that lets you spawn tasks which access borrowed data (poll to completion or "unforgettable" futures are a likely mechanism)
* Less error-prone alternatives to `FuturesUnordered`, generally encouraging the spawning of tasks (and scopes) for non-trivial computation that might time out

## Skill tree

What follows is a "skill tree" that describes the key points of the harmonic synthesis.
Each of the "key goals and points" covered above appears in this skill tree.
